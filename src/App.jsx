...
function App() {
  ...
  const [booksFilteredByPagesRead, setBooksFilteredByPagesRead] =
    useState(books);
  ...
  function updatePrintedBooks() {
    ...
    intersection = intersection.filter((book) =>
      booksFilteredByCategory.includes(book)
    );
    intersection = intersection.filter((book) =>
      booksFilteredByYear.includes(book)
    );
    intersection = intersection.filter((book) =>
      booksFilteredByPages.includes(book)
    );
    intersection = intersection.filter((book) =>
      booksFilteredByPagesRead.includes(book)
    );
    ...
    <div>
      ...
      <div className="libraryApp">
        <div>
          ...
          <button
            onClick={() => setPrintedBooks(books)}
            className="cleanFiltersButton"
          >
            Neteja els filtres
          </button>
          ...
          <div className="filters">
            ...
            <RangeFilter
              ...
              filterMin={0}
              filterMax={2023}
              labelText="Any de publicació: "
              onFilter={updateFilteredBooks}
            />
            <RangeFilter
              books={books}
              filterType="pages"
              filterMin={0}
              filterMax={2000}
              labelText="Número de pàgines: "
              onFilter={updateFilteredBooks}
            />
            <RangeFilter
              books={books}
              filterType="pagesRead"
              filterMin={0}
              filterMax={100}
              labelText="% Pàgines llegides: "
              onFilter={updateFilteredBooks}
            />
          </div>
          <hr />
